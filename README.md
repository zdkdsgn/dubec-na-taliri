...
- **Umí přepnout na jinou školu** — Info → „Vaše škola" nabídne přes 200 škol seskupených podle měst (appka je při prvním spuštění sama nabídne), a živé hledání (přes vlastní Cloudflare proxy, viz `cloudflare/skoly-proxy/`) mezi kýmkoli dalším na jidelna.cz. Výchozí zůstává Dubeč, síťový dotaz navíc jen když si někdo vybere jinou školu
...
- **Spodní menu se schovává** — při scrollu dolů se smrskne do jedné kulaté ikony (aktuální záložky), usazené vlevo dole; při scrollu nahoru nebo klepnutím na ni se zase rozbalí. Ikona se přitom vynoří ze středu obrazovky (odtud, kde pruh menu zmizel) a odjede na své místo – ne obyčejné prolnutí. Horní hlavička byla odstraněna úplně, identitu appky nese ikona a stránka Info
